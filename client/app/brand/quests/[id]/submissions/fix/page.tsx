...
import { useWeb3 } from "@/contexts/fix-useWeb3"
  ...
  useEffect(() => {
    const fetchSubmissions = async ()=>{
      try{
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/brand/mySingleCreatedQuest/${awaitedParams.id}/fix`, {
          credentials: "include",
        });
  ...
  if (!quest) {
    ...
    const handleRewardCreator = async (e: React.FormEvent, creatorAddress: string, questOnChainId: string, pricePerVideo: string, questId: string) => {
     try {
      ...
      // Send tx to db
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/brand/rewardCreator/fix`, {
          method: "POST",
          headers: {
