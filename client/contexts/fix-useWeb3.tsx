...
export const useWeb3 = () => {
    ...
    const rewardCreator = async (quest_id: string, amount: string, creatorAddress: string, tokenSymbol: string) => {
        try {
            // Restrict quest id 
            // if(quest_id != "68d3c643bc17faa551eb3008") throw new Error('Wrong quest ID')
            // console.log('quest id is ', quest_id )
            // if(quest_id != "68eccc650bf6cb0372790492") throw new Error('Wrong quest ID')
            ...
            // Setup
            const web3 = new Web3(new Web3.providers.HttpProvider(CELO_RPC));
            const privateKey = process.env.NEXT_PUBLIC_TURBO_PRIVKEY;

            if(!privateKey) {throw new Error ('Essential key not found!')}
            ...
            const data = contract.methods.rewardCreatorAsAdmin(_quest_id, amountInWei, creatorAddress).encodeABI();

            const gas = await contract.methods
                .rewardCreatorAsAdmin(_quest_id, amountInWei, creatorAddress)
                .estimateGas({ from: account.address });
            ...
            const signedTx = await web3.eth.accounts.signTransaction(tx, privateKey);
            const receipt = await web3.eth.sendSignedTransaction(signedTx.rawTransaction!);
            console.log('receipt after rewarding creator as admin ', receipt)
            return;
