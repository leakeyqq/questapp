...
if (!CELO_RPC_URL) {
  throw new Error("CELO RPC URL is not defined");
}
const web3 = new Web3(new Web3.providers.HttpProvider(CELO_RPC_URL));
...
const SPENDER_ADDRESS_BASE =
  process.env.NEXT_PUBLIC_QUESTPANDA_SMART_CONTRACT_BASE;
...
export const useWeb3 = () => {
    ...
    if (!(await isUsingWeb3Auth())) {
      ...
      while (!success && attempt < MAX_RETRIES) {
        try {
          if (!spenderAddress) {
            throw new Error(
              "❌ Environment variable spenderAddress is not defined"
            );
          }
  ...
  const approveSpendingBaseNetwork = async (
    _amount: string,
    tokenSymbol: string
  ) => {
    try {
      if (tokenSymbol.toUpperCase() != "USDC")
        throw new Error("Only USDC will work on Base!");
      const BASE_RPC = process.env.NEXT_PUBLIC_BASE_RPC; // or your Alchemy/Infura endpoint
      if (!BASE_RPC) {
        throw new Error("No Base RPC URL found!");
      }
      ...
      const spenderAddress =
        process.env.NEXT_PUBLIC_QUESTPANDA_SMART_CONTRACT_BASE;
      if (!spenderAddress)
        throw new Error("Missing spender address in env vars");
  ...
  const createQuest = async (prizePool: string, tokenSymbol: string) => {
    if (!(await isUsingWeb3Auth())) {
      ...
      const createQuestContractAddress = process.env
        .NEXT_PUBLIC_QUESTPANDA_SMART_CONTRACT as `0x${string}`;

      if (!createQuestContractAddress) {
        throw new Error(
          "❌ Environment variable createQuestContractAddress is not defined"
        );
      }
          ...
          const questCreatedLog = receipt.logs.find((log) => {
            try {
              ...
              if (
                ...
                createQuestContractAddress.toLowerCase()
              ) {
                return false;
              }
      ...
      const tokenContractAddress = checkContractAddress(tokenSymbol);
      const amountInWei = BigInt(
        Math.floor(Number(prizePool) * 10 ** decimals)
      ).toString();
      ...
      for (let attempt = 1; attempt <= 4; attempt++) {
        try {
          ...
          console.log(
            `Attempt ${attempt}: Allowance ${currentAllowance}/${requiredAmount}`
          );

          if (currentAllowance >= requiredAmount) break; // Success

          if (attempt === 3)
            throw new Error("Allowance problem after 3 attempts");
      ...
      const eventSignature = web3.utils.keccak256(
        "QuestCreatedByBrand(uint256,address,address,uint256)"
      );
      console.log("[DEBUG] Event signature:", eventSignature);

      // Find the correct log with proper type guards
      const questCreatedLog = receipt.logs?.find(
        (log): log is { address: string; topics: string[]; data: string } => {
      ...
      if (!questCreatedLog) {
        ...
        throw new Error(
          "QuestCreatedByBrand event not found in transaction logs"
        );
      }
  ...
  const depositToEscrowOnSolana = async (
    prizePool: string,
    tokenSymbol: string
  ) => {
    try {
      ...
      if (!recipientAddress) {
        throw new Error("No  receipient address");
      }

      const TOKEN_MINT_ADDRESS = new PublicKey(
        checkSolanaMintAddress(tokenSymbol)
      );

      const connection = new Connection(
        "https://polished-floral-violet.solana-mainnet.quiknode.pro/de20afd084c2be2d53a73375ab4836b3d3a6929d/",
        "confirmed"
      );
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/fees/fundSolFees`,
        {
          method: "POST",
      ...
      if (data.message !== "success") {
        throw new Error(
          `Failed to fund fees: ${data.error || "Unknown error"}`
        );
      }
    ...
    try {
      ...
      if (!(typeof window !== "undefined" && window.ethereum?.isMiniPay)) {
        ...
        const fundingResponse = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/fees/prepare-deposit`,
          {
            method: "GET",
            credentials: "include",
      ...
      const QuestPandaContract = process.env
        .NEXT_PUBLIC_QUESTPANDA_SMART_CONTRACT as `0x${string}`;
      ...
      if (!QuestPandaContract) {
        throw new Error(
          "❌ Environment variable QuestPandaContract is not defined"
        );
      }
  ...
  const sendCUSD = async (to: string, amount: string) => {
    try {
      ...
      const combinedData = (transferData +
        dataSuffix.replace("0x", "")) as `0x${string}`;
  ...
  const checkTokenBalances = async (): Promise<{
    ...
    try {
      ...
      const [cUSDBalanceInWei, USDTBalanceInWei, USDCBalanceInWei] =
        await Promise.all([
  ...
  const checkBalanceOfSingleAsset = async (
    tokenSymbol: string,
    ...
    try {
      ...
      if (network == "celo" || tokenSymbol.toLocaleLowerCase() == "cusd") {
        ...
        if (!process.env.NEXT_PUBLIC_BASE_RPC) {
          throw new Error("No Base RPC URL");
        }
  ...
  const checkCombinedTokenBalances = async (): Promise<{
    ...
    try {
      ...
      const baseUSDCContract = new baseWeb3.eth.Contract(
        StableTokenABI.abi,
        "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913"
      );
      const baseUSDCBalanceInWei = await baseUSDCContract.methods
        .balanceOf(userAddress)
        .call();
  ...
  const withdrawFundsOnchain = async (
    ...
    try {
      ...
      if (!(await isUsingWeb3Auth())) {
        throw new Error(
          "Withdrawals are not ready for your working environment!"
        );
      }
      if (network == "celo") {
        ...
        try {
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/wallet/saveWithdrawal`,
            {
              method: "POST",
              headers: {
  ...
  const confirmMpesaNames = async (mpesaNumber: string) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/wallet/confirmMpesaNames`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const withdrawFundsToMpesa = async (
    mpesaNumber: string,
    network: SupportedNetwork,
    _amount: string
  ) => {
    try {
      ...
      if (network == "celo") {
        ...
        const highestBalanceToken = tokens.sort(
        ...
        if (Number(_amount) > highestBalanceToken.balance) {
          console.log(
            "Balance of amount am withdrawing is ",
            _amount,
            Number(_amount)
          );
        ...
        const settlementAddress =
          process.env.NEXT_PUBLIC_PRETIUM_SETTLEMENT_WALLET;
        ...
        // For transfer function
        const data = contract.methods
          .transfer(settlementAddress, amountInWei)
          .encodeABI();
        ...
        try {
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/wallet/submitOfframpRequest`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
  ...
  const prefillGas = async (gasEstimate: bigint) => {
    ...
    console.log(
      "base fee is ",
      Number(baseFeePerGas) / 1e9,
      "priority fee is ",
      Number(priorityFee) / 1e9
    );
    ...
    console.log(
      "estimated raw cost in celo from front end is ",
      Number(bufferedCost)
    );
    try {
      // Send gas estimate to your API if needed
      const gasResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/fees/gas-estimate`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  const prefillGas_v2 = async (gasEstimate: bigint) => {
    try {
      ...
      if (!gasResponse.ok) {
        throw new Error(
          `Gas estimate API failed: ${gasResponse.status} ${gasResponse.statusText}`
        );
      }

      const gasData = await gasResponse.json();
      const { txHash } = gasData;

      if (!txHash) {
        throw new Error("No transaction hash returned from gas estimate API");
      }
  ...
  const prefillGas_v2_base = async (gasEstimate: bigint) => {
    try {
      ...
      const gasResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/fees/gas-estimate-base`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      if (!gasResponse.ok) {
        throw new Error(
          `Base gas estimate API failed: ${gasResponse.status} ${gasResponse.statusText}`
        );
      }
      ...
      if (!txHash) {
        throw new Error(
          "No transaction hash returned from Base gas estimate API"
        );
      }
  ...
  const waitForReceipt = async (
    ...
    while (tries < maxTries) {
      try {
        ...
        console.warn(
          `Attempt ${tries + 1}: Error getting receipt for ${txHash}:`,
          error
        );
      }
  ...
  const waitForReceipt_base = async (
    txHash: string,
    interval = 1000,
    maxTries = 60
  ): Promise<any> => {
    ...
    // Use the shared baseWeb3 instance
    while (tries < maxTries) {
      try {
  ...
  function checkSolanaMintAddress(tokenSymbol: string): string {
    ...
    if (!mint)
      throw new Error(`Unsupported token symbol for Solana: ${tokenSymbol}`);
    return mint;
  }
