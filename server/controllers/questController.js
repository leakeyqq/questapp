...
export const getAllQuests = async (req, res) => {
  try {
    ...
    const allQuests = await Quest.find({ visibleOnline: true }, { brandName: 1, brandImageUrl: 1, description: 1, prizePoolUsd: 1, endsOn: 1, pricePerVideo: 1, onchain_id: 1, rewardToken: 1, approvalNeeded: 1, questType: 1, socialPlatformsAllowed: 1 }).sort({ createdAt: -1 }).lean().exec()
...
export const get3questsOnly = async (req, res) => {
  try {
    const _3quests = await Quest.find({ visibleOnline: true }, { brandName: 1, brandImageUrl: 1, description: 1, prizePoolUsd: 1, endsOn: 1, pricePerVideo: 1, onchain_id: 1, rewardToken: 1, approvalNeeded: 1, questType: 1, socialPlatformsAllowed: 1 }).sort({createdAt: -1 }).limit(3).lean().exec()
...
if (Number(data.core.user_results.result.legacy.followers_count) < Number(minFollowersRequired)){
  ...
  if (creatorData_twitter) {
    ...
    try {
      ...
      const _creator = await Creator.findOneAndUpdate(
        { creatorAddress: walletID },
        {
          $inc: {
