...
export const submitOfframpRequest = async (req, res) => {
    try {
        ...
        // Get pretium exchange rate
        const response_rates = await fetch(`${PRETIUM_BASE_URI}/v1/exchange-rate`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'x-api-key': `${PRETIUM_API_KEY}`
        ...
        const KES_FX_RATE = data_rates.data.buying_rate
